...
def build_turns(messages: List[Dict[str, Any]]) -> List[Turn]:
    ...
    def flush_turn():
        nonlocal current_user, assistant_order, assistant_latest, tool_results_by_id, tool_uses_by_id, tool_result_timestamps, msg_tool_use_ids, last_tool_result_id, turns
        if current_user is None:
            return
        if not assistant_latest:
            return
        assistants = [assistant_latest[mid] for mid in assistant_order if mid in assistant_latest]
        turns.append(Turn(user_msg=current_user, assistant_msgs=assistants, tool_results_by_id=dict(tool_results_by_id), tool_uses_by_id=dict(tool_uses_by_id), tool_result_timestamps=dict(tool_result_timestamps), msg_tool_use_ids=dict(msg_tool_use_ids)))
        ...
        with langfuse.start_as_current_observation(
            ...
            for resp_idx, asst_msg in enumerate(turn.assistant_msgs):
                ...
                if resp_idx == 0:
                    ...
                    if prev_tus:
                        last_prev_tid = str(prev_tus[-1].get("id", ""))
                        asst_start_ns = turn.tool_result_timestamps.get(last_prev_tid) or get_timestamp_ns(asst_msg)
                    else:
                        asst_start_ns = get_timestamp_ns(asst_msg)
                ...
                for tu in tool_uses:
                    ...
                    with turn_span.start_as_current_observation(
                        ...
                        tool_obs.update(output=out_trunc)
